...
const AddNoteForm = () => {
  ...
  const onSaveNoteClicked = () => {
    if (!titleError && !contentError){
      ...
      toast("new note successfully created");
    ...
    <div>
      ...
      <form className='note-form'>
        ...
        <div className='form-element'>
          ...
          onChange={onFormDataChange} className='form-control' rows='10' value={formData.noteContent}></textarea>
          <span className='form-error-text'>
            {contentError ? "Content cant be empty" : ""} 
          </span>
        </div>
        <button type='button' onClick={(onSaveNoteClicked)} className='btn btn-default' 
